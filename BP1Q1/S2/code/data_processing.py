...
mypath = os.getcwd()
city = 'Chicago'
mypath1 = mypath + '\\Weather\\' + city + '\\Chicago.csv'
...
for f in onlyfiles:
    ...
    ind = data.index[data['Date & Time'] == '2015-12-31 21:00'].tolist() + \
        data.index[data['Date & Time'] == '2015-01-01 00:00'].tolist()
    if not ind:
        ind = data.index[data['Date & Time'] == '2015-12-31 21:00:00'].tolist() + \
            data.index[data['Date & Time'] == '2015-01-01 00:00:00'].tolist()
    if not ind:
        ind = data.index[data['Date & Time'] == '12/31/2015 21:00'].tolist() + \
            data.index[data['Date & Time'] == '1/1/2015 0:00'].tolist()            
...
temp = [datetime.strptime(t, "%Y-%m-%d %H:%M") for t in datetimes]
