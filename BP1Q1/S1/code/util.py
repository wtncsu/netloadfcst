# -*- coding: utf-8 -*-
"""
Created on Sat Sep  4 20:11:30 2021
...
def data_loader(name):
    ...
    maximum, minimum = max(Data['Demand']), min(Data['Demand'])
    Data['Demand'] = Data['Demand'].apply(lambda x: (x-minimum) / (maximum - minimum))
    Data['Past1day'], Data['Past1week'] = Data['Demand'].shift(periods=24).fillna(0), \
        Data['Demand'].shift(periods=24*7).fillna(0)
...
def LSTM_base(Y_train, Y_test, X_train, X_test, minimum, maximum):
    ...
    model.fit(X_train, Y_train, batch_size=32, epochs=50, validation_split=0.2, verbose=0)
...
if __name__ == "__main__":
    ...
    Data, minimum, maximum = data_loader('../data_cleaned/data.csv')