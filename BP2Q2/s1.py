...
markdown_text = '''
### S1: 1-Year Data of 48 Houses in the Denver Area
...
layout = html.Div([
    ...
    html.Div(
        dbc.Button('Download', id='s1-download'),
        style={'display': 'inline-block', 'margin-left': '20px'}
    ),
    dcc.Download(id='s1-download-csv'),
    html.Div(
        dcc.Upload(
    ...
    dcc.Graph(id='s1-eval')
])

@app.callback(Output('s1-load-ts', 'figure'), Input('s1-load-ts-res', 'value'), prevent_initial_call=True)
def update_s1_load_ts(res):
    if res == 'Hourly':
...
@app.callback(Output('s1-load-stat', 'figure'), Input('s1-load-stat-by', 'value'), prevent_initial_call=True)
def update_s1_load_stat(res):
    if res == 'By hour':
...
@app.callback(Output('s1-load-temp', 'figure'), Input('s1-load-temp-by', 'value'), prevent_initial_call=True)
def update_s1_load_temp(res):
    if res == 'By hour':
        fig = px.scatter(
...
@app.callback(
    Output('s1-download-csv', 'data'),
    Input('s1-download', 'n_clicks'),
    State('s1-eval-test', 'start_date'),
    State('s1-eval-test', 'end_date'),
    prevent_initial_call=True
)
def update_s1_download(n_clicks, start_date, end_date):
    df_test = df[start_date:end_date]
    return dcc.send_data_frame(df_test.to_csv, 'test.csv')

@app.callback(
    Output('s1-eval', 'figure'),
    Input('s1-upload', 'contents'),
    State('s1-eval-test', 'start_date'),
    State('s1-eval-test', 'end_date'),
    prevent_initial_call=True
)
def update_s1_upload(contents, start_date, end_date):
