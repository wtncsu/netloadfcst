...
markdown_text = '''
### L1: 5-Year Data of 3 Areas in PJM
...
layout = html.Div([
    ...
    html.Div(
        dbc.Button('Download', id='l1-download'),
        style={'display': 'inline-block',"margin-left": "20px"}
        ),
    dcc.Download(id='l1-download-csv'),
    html.Div(    
        dcc.Upload(
            id='l1-upload',
            children=html.Div([
    ...
    dcc.Graph(id='l1-eval')
])

@app.callback(Output('l1-load-ts', 'figure'), Input('l1-load-ts-res', 'value'), prevent_initial_call=True)
def update_l1_load_ts(res):
    if res == 'Hourly':
...
@app.callback(Output('l1-load-stat', 'figure'), Input('l1-load-stat-by', 'value'), prevent_initial_call=True)
def update_l1_load_stat(res):
    if res == 'By hour':     
...
@app.callback(Output('l1-load-temp', 'figure'), Input('l1-load-temp-by', 'value'), prevent_initial_call=True)
def update_l1_load_temp(res):
    if res == 'By hour':
        fig = px.scatter(
...
@app.callback(
    Output('l1-download-csv', 'data'),
    Input('l1-download', 'n_clicks'),
    State('l1-eval-test', 'start_date'),
    State('l1-eval-test', 'end_date'),
    prevent_initial_call=True
)
def update_l1_download(n_clicks, start_date, end_date):
    df_test = df[start_date:end_date]
    return dcc.send_data_frame(df_test.to_csv, 'test.csv')

@app.callback(
    Output('l1-eval', 'figure'),
    Input('l1-upload', 'contents'),
    State('l1-eval-test', 'start_date'),
    State('l1-eval-test', 'end_date'),
    prevent_initial_call=True
)
def update_l1_upload(contents, start_date, end_date):
