...
markdown_text = '''
### S4: 2-Year Data of 3 Areas in PJM

The data are collected from the [PJM’s Data Miner 2] (https://dataminer2.pjm.com).
The PJM dataset includes hourly energy data (net meter and solar generation) for areas in the PJM market. 
The 3 selected areas are MIDATL, SOUTH, and WEST. Data from 01/01/2019 to 06/30/2020 are used for training, 
and data from 07/01/2020 to 12/31/2020 are used for testing.
'''

DATA_PATH = os.path.join( "data/s4")
...
layout = html.Div([
    ...
    html.Div(
        dbc.Button('Download', id='s4-download'),
        style={'display': 'inline-block',"margin-left": "20px"}
        ),
    dcc.Download(id='s4-download-csv'),
    html.Div(    
        dcc.Upload(
            id='s4-upload',
            children=html.Div([
    ...
    dcc.Graph(id='s4-eval')
])

@app.callback(Output('s4-load-ts', 'figure'), Input('s4-load-ts-res', 'value'), prevent_initial_call=True)
def update_s4_load_ts(res):
    if res == 'Hourly':
...
@app.callback(Output('s4-load-stat', 'figure'), Input('s4-load-stat-by', 'value'), prevent_initial_call=True)
def update_s4_load_stat(res):
    if res == 'By hour':     
...
@app.callback(Output('s4-load-temp', 'figure'), Input('s4-load-temp-by', 'value'), prevent_initial_call=True)
def update_s4_load_temp(res):
    if res == 'By hour':
        fig = px.scatter(
...
@app.callback(
    Output('s4-download-csv', 'data'),
    Input('s4-download', 'n_clicks'),
    State('s4-eval-test', 'start_date'),
    State('s4-eval-test', 'end_date'),
    prevent_initial_call=True
)
def update_s4_download(n_clicks, start_date, end_date):
    df_test = df[start_date:end_date]
    return dcc.send_data_frame(df_test.to_csv, 'test.csv')

@app.callback(
    Output('s4-eval', 'figure'),
    Input('s4-upload', 'contents'),
    State('s4-eval-test', 'start_date'),
    State('s4-eval-test', 'end_date'),
    prevent_initial_call=True
)
def update_s4_upload(contents, start_date, end_date):
    df_test = df[start_date:end_date].copy()
    if contents is None:
        fig = px.line(df_test, x=df_test.index, y='Load', color='Location', labels={'Load': 'Load (kW)'})
    else:
        df_test['set'] = 'Actual'
        ...
        fig = px.line(df_eval, x=df_eval.index, y='Load', color='Location', line_dash = 'set', labels={'Load': 'Load (kW)'}, title = title)
