...
markdown_text = '''
### L4: 5-Year Data of 8 Areas in NYISO

The data are collected from the [NYISO] (https://www.nyiso.com/load-data).
NYISO does not provide solar data, the solar data are also collected from [PVWatts] (https://pvwatts.nrel.gov/). 
The temperature data are collected from [NOAA] (https://www.noaa.gov/). 
The 8 selected areas are CAPITAL, CENTRAL, DUNWOD, GENESE, HUD VL, LONGIL, MILLWD, and NYC. 
Data from 01/01/2015 to 12/31/2018 are used for training, and data from 01/01/2019 to 12/31/2019 are used for testing.
'''

DATA_PATH = os.path.join( "data/l4")
...
layout = html.Div([
    ...
    html.Div(
        dbc.Button('Download', id='l4-download'),
        style={'display': 'inline-block',"margin-left": "20px"}
        ),
    dcc.Download(id='l4-download-csv'),
    html.Div(    
        dcc.Upload(
            id='l4-upload',
            children=html.Div([
    ...
    dcc.Graph(id='l4-eval')
])

@app.callback(Output('l4-load-ts', 'figure'), Input('l4-load-ts-res', 'value'), prevent_initial_call=True)
def update_l4_load_ts(res):
    if res == 'Hourly':
...
@app.callback(Output('l4-load-stat', 'figure'), Input('l4-load-stat-by', 'value'), prevent_initial_call=True)
def update_l4_load_stat(res):
    if res == 'By hour':     
...
@app.callback(Output('l4-load-temp', 'figure'), Input('l4-load-temp-by', 'value'), prevent_initial_call=True)
def update_l4_load_temp(res):
    if res == 'By hour':
        fig = px.scatter(
...
@app.callback(
    Output('l4-download-csv', 'data'),
    Input('l4-download', 'n_clicks'),
    State('l4-eval-test', 'start_date'),
    State('l4-eval-test', 'end_date'),
    prevent_initial_call=True
)
def update_l4_download(n_clicks, start_date, end_date):
    df_test = df[start_date:end_date]
    return dcc.send_data_frame(df_test.to_csv, 'test.csv')

@app.callback(
    Output('l4-eval', 'figure'),
    Input('l4-upload', 'contents'),
    State('l4-eval-test', 'start_date'),
    State('l4-eval-test', 'end_date'),
    prevent_initial_call=True
)
def update_l4_upload(contents, start_date, end_date):
