...
markdown_text = '''
### L3: 5-Year Data of 14 Countries in Europe
...
We consider the aggregated net load as our target. Data from 01/01/2015 to 12/31/2018 are used for training,
and data from 01/01/2019 to 12/31/2019 are used for testing.

'''

DATA_PATH = os.path.join( "data/l3")
...
layout = html.Div([
    ...
    html.Div(
        dbc.Button('Download', id='l3-download'),
        style={'display': 'inline-block',"margin-left": "20px"}
        ),
    dcc.Download(id='l3-download-csv'),
    html.Div(    
        dcc.Upload(
            id='l3-upload',
            children=html.Div([
    ...
    dcc.Graph(id='l3-eval')
])

@app.callback(Output('l3-load-ts', 'figure'), Input('l3-load-ts-res', 'value'), prevent_initial_call=True)
def update_l3_load_ts(res):
    if res == 'Hourly':
...
@app.callback(Output('l3-load-stat', 'figure'), Input('l3-load-stat-by', 'value'), prevent_initial_call=True)
def update_l3_load_stat(res):
    if res == 'By hour':     
...
@app.callback(Output('l3-load-temp', 'figure'), Input('l3-load-temp-by', 'value'), prevent_initial_call=True)
def update_l3_load_temp(res):
    if res == 'By hour':
        fig = px.scatter(
...
@app.callback(
    Output('l3-download-csv', 'data'),
    Input('l3-download', 'n_clicks'),
    State('l3-eval-test', 'start_date'),
    State('l3-eval-test', 'end_date'),
    prevent_initial_call=True
)
def update_l3_download(n_clicks, start_date, end_date):
    df_test = df[start_date:end_date]
    return dcc.send_data_frame(df_test.to_csv, 'test.csv')

@app.callback(
    Output('l3-eval', 'figure'),
    Input('l3-upload', 'contents'),
    State('l3-eval-test', 'start_date'),
    State('l3-eval-test', 'end_date'),
    prevent_initial_call=True
)
def update_l3_upload(contents, start_date, end_date):
